...
const seed = async () => {
  ...
  const records = await Record.insertMany([
    ...
    { title: 'Software License Audit',  description: 'Audit all software licenses company-wide',status: 'Active',  priority: 'Low',    assignedTo: 'user001', createdBy: 'admin001', category: 'IT Support' },
  ...
  console.log('\n📋 Login Credentials:');
  console.log('  Admin  → userId: admin001 | password: Admin@123 | role: Admin');
  console.log('  User 1 → userId: user001  | password: User@123  | role: General User');
  console.log('  User 2 → userId: user002  | password: User@123  | role: General User');

  await mongoose.disconnect();
  console.log('\n✅ Seeding complete. Disconnected.');
};
